...
namespace EgyptianWar
{
    ...
    public class CardAnimator : MonoBehaviour
    {
        ...
        public void AddCardToPool(Player player, Pool pool, byte value)
        {
            int numberOfDisplayingCard = DisplayingCards.Count;
            Card card = player.GiveTopCard();
            card.SetCardValue(value);
            card.SetFaceUp(true);
            pool.ReceivePlayerCard(card);
            card.SetDisplayingOrder(Order);
            Order++;
            AddCardAnimation(card, pool.Position);

            //Card card = DisplayingCards[numberOfDisplayingCard - 1];
            //DisplayingCards.Add(card);
        }

        public void AddCardToPlayer(Player player, Pool pool, byte value)
        {
            int numberOfDisplayingCard = DisplayingCards.Count;
            Card card = pool.GiveTopCard(player);
            if (card == null) {
                return;
            }
            card.SetCardValue(value);
            ...
            player.ReceiveDisplayingCard(card);
            card.SetDisplayingOrder(-1);
            AddCardAnimation(card, player.Position);
        }
